...
<head>
	...
	<link href="https://fonts.googleapis.com/css2?family=Bokor&amp;family=Staatliches&amp;display=swap" rel="stylesheet">
...
<body id="phpbb">
	...
	<ul id="generated_nav_bar" style="display: none;">
        <li>{GENERATED_NAV_BAR}</li>
    </ul>

	<div id="page-nav">
		<div id="online-avatar"></div>
		
		<div class="nav"><a href="{U_INDEX}" alt="accueil" title="accueil">accueil</a>
			&nbsp;&nbsp;<a href="/memberlist" alt="liste des membres" title="liste des membres">membres</a>
			&nbsp;&nbsp;<a href="/t4057-liens-utiles#99068" alt="sujet des liens utiles" title="sujet des liens utiles">liens utiles</a>
			&nbsp;&nbsp;<a href="/search" alt="recherche" title="recherche" onclick="showhide(document.getElementById('search_menu')); return false;">recherche</a>
			&nbsp;&nbsp;<a href="/groups" alt="liste des groupes" title="liste des groupes">groupes</a>
			<!-- BEGIN switch_user_logged_in -->
			&nbsp;&nbsp;<a href="/profile?mode=editprofile" alt="profil" title="profil">profil</a>
			&nbsp;&nbsp;<a id="mp" href="/privmsg?folder=inbox" alt="messagerie" title="messagerie">messagerie</a>
			&nbsp;&nbsp;<a href="/login?logout=1" alt="déconnexion" title="déconnexion">déconnexion</a>
 	...
 	<script type="text/javascript">
		...
		jQuery().ready(function(){
			...
			if(!e.length){ return; } else {
				var n= +e.attr("title").replace(/^([1-9][0-9]*) .*$/,"$1");
				var title = n+" nouveau"+(n>1?"x":"")+" message"+(n>1?"s":"");
				$("#mp").addClass("new_mp");
				$("#mp").text(title);
				$("#mp").attr("alt", title);
				$("#mp").attr("title", title);
	...
	<div id="page-cadre-right"></div>
	<div id="page-cadre-left"></div>
	<div id="page-cadre-bottom"></div>
	...
	<script>
		const element = document.getElementById("search_menu");
		element.remove();
	</script>
       ...
       <!-- ENTETE DU FORUM (au-dessus de la pub) -------------->
        ...
        <div id="page-header">
           <!-- Bannière -->
			...
			<div id="header">
				<div id="header_img">
					<a href="{U_INDEX}" id="header_link"></a>
				</div>
			</div>
        </div>
        <!-- Fin de l'entête -->
