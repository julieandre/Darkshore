  ...
  var FA_TOOLBAR = "fa_toolbar",
    ...
    init = function (tbParams) {
      ...
      if (_userdata["activate_toolbar"]) {
        ...
        if (
          _userdata["tpl_mobile"] == "mobi_modern" ||
          _userdata["tpl_used"] == "awesomebb"
        ) {
          $("#" + LIVE_NOTIF).css(
            "right",
            $("#" + HIDE).outerWidth(true) - 1 + "px"
          );
          ...
          $(document).click(function (e) {
            switch (e.target.id) {
              ...
              default:
                try {
                  if (document.getElementById(NOTIF_LIST)) {
                    if (
                      jQuery.contains(
        ...
        $(document).click(function (e) {
          ...
          var options =
            "menubar=no, status=no, scrollbars=no, width=800, height=600";
          switch (e.target.id) {
            case MAGNIFIER:
    ...
    _createLeft = function () {
      ...
      icon.id = ICON;
      a.id = SERVICE;
      typeof a.innerText == "undefined"
        ? (a.textContent = _board["Forumotion"])
        : (a.innerText = _board["Forumotion"]);
    ...
    _createShare = function () {
      ...
      typeof shareText.innerText == "undefined"
        ? (shareText.textContent = _lang["Share"] + " : ")
        : (shareText.innerText = _lang["Share"] + " : ");
      fb.id = FB;
      twitter.id = TWITTER;
      mail.id = MAIL;
    ...
    _createRight = function () {
      ...
      if (!_userdata["session_logged_in"]) {
        ...
        typeof login.innerText == "undefined"
          ? (login.textContent = _lang["Login"])
          : (login.innerText = _lang["Login"]);
        login.href =
          _internalProtocol + "//" + window.location.hostname + "/login";
        typeof register.innerText == "undefined"
          ? (register.textContent = _lang["Register"])
          : (register.innerText = _lang["Register"]);
        register.href =
          _internalProtocol + "//" + window.location.hostname + "/register";
        ...
        var divMenu = document.createElement("div"),
          ...
          menu = _createMenu(),
          notifications = _userdata["notifications"]
        ...
        welcome.id = WELCOME;
        $(welcome).html(
          _dynTruncateUsername(
            _lang["Welcome"] + " " + _userdata["username"],
            150
          )
        );
        if (notifCase === 1) {
          ...
          typeof notifications.innerText == "undefined"
            ? (notifications.textContent = _lang["Notifications"])
            : (notifications.innerText = _lang["Notifications"]);
          notif_unread.id = NOTIF_UNREAD;
          notifications.appendChild(notif_unread);
          notif_list.id = NOTIF_LIST;
          notif_view.className = "see_all";
          notif_view_link.href =
            "/profile?mode=editprofile&page_profil=notifications";
          notif_list
            .appendChild(notif_view)
            .appendChild(notif_view_link)
            .appendChild(document.createTextNode(_lang["Notif_see_all"]));
          live_notif.id = LIVE_NOTIF;
          $(live_notif).css({ width: "330px", position: "absolute" });
        } else if (notifCase === 2) {
          notifications.className = "rightHeaderLink";
    ...
    _createMenu = function () {
      ...
      var see = _createSubMenu(
        _lang["See_my_profile"],
        _internalProtocol +
          "//" +
      ...
      var followed = _createSubMenu(
        _lang["js_topics_followed"],
        _internalProtocol +
          "//" +
          window.location.hostname +
          "/search?search_id=watchsearch"
      );
      var separator2 = document.createElement("li");
      var logout = _createSubMenu(
        _lang["Logout"],
        _internalProtocol + "//" + window.location.hostname + "/login?logout=1"
      );
      var mps = _createSubMenu(
        _lang["All_PMs"],
        _internalProtocol +
          "//" +
          window.location.hostname +
      ...
      if (_userdata["user_level"] == "1") {
        var pa = _createSubMenu(
          _lang["Admin_panel"],
          _internalProtocol + "//" + window.location.hostname + "/admin"
        );
        menu.appendChild(pa);
      }
    ...
    _createUserMenu = function () {
      ...
      var rankTitle =
        '<a id="fa_ranktitle" title="' +
        _lang["rank_title"].replace(/<\/?[^>]+>/gi, "") +
        '">' +
        _lang["rank_title"] +
        "</a>";
      ...
      table +=
        '<td class="bold">' + _userdata["user_nb_privmsg"] + "</td></tr>";
      if (_board["reputation_active"]) {
        table +=
          '<tr><td class="first">' + _lang["Reputation"] + "</td><td> : </td>";
        table +=
          '<td class="bold">' + _userdata["point_reputation"] + "</td></tr>";
      }
      table += "</tbody></table>";
  ...
  (_manageStateAndCSS = function () {
    if (_userdata["fix_toolbar"]) {
      if ($.cookie(TOOLBAR_STATE) == HIDE) {
        $("#" + FA_TOOLBAR).css({ marginTop: 0 - _config.tbHeight + "px" });
        $("#" + FA_TOOLBAR_HIDDEN).css({
          marginTop: "+=" + 2 * _config.tbHeight + "px",
        });
      } else {
        $("body").css({ marginTop: _config.tbHeight + "px" });
    ...
    (_moveDown = function () {
      if ($.cookie(TOOLBAR_STATE) && $.cookie(TOOLBAR_STATE) == HIDE) {
        ...
        if (_userdata["fix_toolbar"]) {
          $("body").animate({ marginTop: _config.tbHeight + "px" });
        }
        $("#" + FA_TOOLBAR_HIDDEN).animate({
          marginTop: "-=" + 2 * _config.tbHeight + "px",
        });
    ...
    (_delItem = function (i, uiAction) {
      ...
      if (uiAction) {
        FA.Notification.delItem({ index: i });
      }
    ...
    (_addItem = function (i, data) {
      ...
      if (_text) {
        ...
        _divtext.appendChild(
          document.createTextNode("#" + i + " " + JSON.stringify(data))
        );
      }
      ...
      $(_maindiv.appendChild(document.createElement("a")))
        .addClass("delete")
        .attr("href", "#");
      if (_userdata["tpl_used"] === "awesomebb") {
        let _divtime = document.createElement("div");
      ...
      $("#" + NOTIF_LIST + " li:nth-child(" + i + ")").before(
        $(_dummy_notif)
          .hide()
          .fadeIn(200, function () {
            _alignNotifications();
    ...
    (_readItem = function (i) {
      ...
      $("#" + NOTIF_LIST + " li:nth-child(" + (i + 1) + ")").removeClass(
        "unread"
      );
    }),
    (refresh = function (o) {
        ...
        if (o.map.length > 0) {
          ...
          for (_i in o.map) {
            ...
            if (o.map[_i] === null) {
              ...
              FA.Debugger.log(
                o.data[o.set[_i]].read,
                !o.data[o.set[_i]].read,
                o.data[o.set[_i]].read == 1
              );
          ...
          if ($("#no-notifications").length === 0) {
            ...
            $("#" + NOTIF_LIST + " li#notif-view-all").before(_noNotif);
          } else {
    ...
    (_setConfig = function (data) {
      ...
      _config = $.extend({ tbHeight: 30, tbTemplate: "" }, data);
    ...
    (compileNotif = function (data) {
      ...
      switch (data.text.type) {
        ...
        case FA.Notification.NOTIF_TOPIC_WATCH:
          ...
          FA.Debugger.log(
            "compileNotif()",
            _lang["Notif_topic_watch_p"],
            _lang["Notif_topic_watch"],
            _args
          );
          _text =
            jQuery.vsprintf(
              _args.start
                ? _lang[
                    "Notif_topic_watch_p" +
                      (_args.id == -1 ? "_guest" : "") +
                      (typeof _args.topic_title != "undefined" ? "2" : "")
                  ]
                : _lang[
                    "Notif_topic_watch" +
                      (_args.id == -1 ? "_guest" : "") +
        ...
        case FA.Notification.NOTIF_ADVERT:
          ...
          _text =
            jQuery.vsprintf(_lang["Notif_advert_" + data.text.post.type], [
              _args,
            ]) || false;
          break;
        case FA.Notification.NOTIF_FORUM_WATCH:
          ...
          break;
        case FA.Notification.NOTIF_FOLLOWER_NEW_TOPIC:
          _args = {
            id: data.text.from.id,
            name: data.text.from.name,
            topic_id: data.text.post.topic_id,
          ...
          _text =
            jQuery.vsprintf(_lang["Notif_followers_new_topic"], [_args]) ||
            false;
          break;
        case FA.Notification.NOTIF_FOLLOWER_NEW_POST:
          _args = {
            id: data.text.from.id,
            name: data.text.from.name,
            topic_id: data.text.post.topic_id,
          ...
          _text =
            jQuery.vsprintf(
              _args.start
                ? _lang[
                    "Notif_followers_new_post_p" +
                      (typeof _args.topic_title != "undefined" ? "2" : "")
                  ]
                : _lang[
                    "Notif_followers_new_post" +
                      (typeof _args.topic_title != "undefined" ? "2" : "")
                  ],
              [_args]
            ) || false;
...
var _toolbar = { notifications: { unread: 0 } };
